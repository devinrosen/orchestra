...
- [implemented] **Smart playlists** — Rule-based auto-playlists that update dynamically (e.g., "genre = Jazz AND year > 2000", "added in last 30 days", "bitrate > 256 AND format = FLAC"). Define rules in a builder UI, results are computed from SQLite queries. Combine with ratings/favorites for powerful personal curation.
