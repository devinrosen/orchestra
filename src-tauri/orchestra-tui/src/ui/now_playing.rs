...
pub fn render(f: &mut Frame, area: Rect, now_playing: &Option<NowPlaying>, volume: f32, status_msg: &Option<String>) {
    let vol_pct = (volume * 100.0).round() as u32;

    let content: Line = if let Some(err) = status_msg {
        ...
        Line::styled(
            "  No track playing  (Enter = play, Space = pause, n/p = next/prev, +/- = volume, q = quit)",
